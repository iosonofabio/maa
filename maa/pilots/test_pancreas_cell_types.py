...
content:    Try to see where in the sorting plots are successful and failed
            cells for different pancreas cell types (after RNA-Seq annotation).
'''
# Modules
import os
import sys
...
def parse_facs_map():
    fn = '../../data/index_fcs_pancreas/FACSmap.csv'
...
if __name__ == '__main__':
    ...
    plates_pancreas = np.unique(facs_map['plate'])

    facs_data = {}
    for ip, plate in enumerate(plates_pancreas):
        ...
        #for groupby in ('Good quality', 'cell type call'):
        for groupby in ('cell type call',):
            ...
            fig.savefig('../../figures/{:}.png'.format(plate))
            plt.close(fig)
